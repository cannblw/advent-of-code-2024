...
const tokensA = 3
const tokensB = 1
const calculationError float64 = 10000000000000
...
func main() {
	...
	for scanner.Scan() {
		...
		case Prize:
			p1, err = strconv.ParseFloat(matches[1], 64)
			if err != nil {
				log.Fatalf("Error converting item %s to float: %v", matches[1], err)
			}
			p2, err = strconv.ParseFloat(matches[2], 64)
			if err != nil {
				log.Fatalf("Error converting item %s to float: %v", matches[2], err)
			}
			...
			b := mat.NewVecDense(2, []float64{p1, p2})
